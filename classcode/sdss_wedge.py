...
# Input catalogues
from .setupdir import data_home_dir
path = data_home_dir()

[ra, dec, z], [ra_isol, dec_isol, z_isol], [ra_pair, dec_pair, z_pair], [ra_trip, dec_trip, z_trip] = [np.loadtxt(path+filename+'.txt', usecols = (0, 1, 2), unpack=True) for filename in ['/SDSS_DR10_galaxy_local', '/table1', '/table2', '/table3']]
...
decCen0, decDelta0, alpha0 = 0., 10., .7

# Main plot: Wedge diagram
fig = plt.figure("Wedge diagram", figsize=(16.3, 8.4))
ax1, ax2 = [plt.axes(posval, polar=val, projection=pj) for posval, val, pj in zip([[0.05, 0.2, 0.9, 0.7], [.68, 0.2, 0.25, 0.25]], [True, False], [None, 'mollweide'])]
[ax.grid(True) for ax in [ax1, ax2]]
# set redshift labels along vertical axis
ax1.set_rlabel_position(250)

# Plotting galaxy positions in the selected declination range
xyplt = [ax1.plot(rad_tot[i][cond_dec(decCen0, decDelta0)[i]], z_tot[i][cond_dec(decCen0, decDelta0)[i]], krgb, ms=mval, alpha=alpval, visible=visi)[0] for i, krgb, mval, alpval, visi in zip(range(0, 4), ['k.', 'ro', 'go', 'bo'], [1, 4, 4, 4], [alpha0, .7, .7, .7], [True, False, False, False])]
...
check = CheckButtons(rax, labels, (True, False, False, False))
def func(label):
